...
PROJECT="reForis Snapshots Plugin"
...
create-messages:
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_snapshots/translations/messages.pot . --project=$(PROJECT) --version=$(VERSION) --copyright-holder=$(COPYRIGHT_HOLDER) --msgid-bugs-address=$(MSGID_BUGS_ADDRESS)

.PHONY: update-messages
update-messages:
	$(VENV_BIN)/pybabel update -i ./reforis_snapshots/translations/messages.pot -d ./reforis_snapshots/translations --update-header-comment
